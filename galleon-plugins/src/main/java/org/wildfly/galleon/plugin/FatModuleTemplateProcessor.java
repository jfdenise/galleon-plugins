...
/**
 * A Template processor that process templates when provisioning fat server. In this case
 * artifacts are copied to the modules directory and module.xml reference local artifact.
 *
 * @author jdenise
 */
class FatModuleTemplateProcessor extends ModuleTemplateProcessor {

    public FatModuleTemplateProcessor(WfInstallPlugin plugin, ModuleArtifactInstaller installer, Path targetPath, ModuleTemplate template, Map<String, String> versionProps) {
    ...
    protected void processArtifact(ModuleArtifact artifact) throws IOException, MavenUniverseException, ProvisioningException {
        ...
        if (artifact.isJandex()) {
            ...
            finalFileName = getInstaller().installArtifactFat(artifact.getMavenArtifact(), artifactFileName, getTargetDir());
        }
